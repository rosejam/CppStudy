...
int main()
{
    ...
    // - [배열의 이름]만 따로 있는 경우 배열의 시작 주소값을 가지는/시작 위치를 가리키는 TYPE*형 포인터로 항상 변환!!!!!!!
    // - (이 때 TYPE은 배열의 데이터 타입이다.)
    pointer = array;
    // -- (배열을 함수 인자로 넘길 때도 자연스럽게 포인터 타입으로 변환해서 넘긴다!!!!)
    ...
    cout << *(array + 5) << endl;
    cout << endl;
    cout << pointer[10] << endl; // 범위 밖도 접근 가능;!!!
    cout << array[10] << endl;
    // -- (C++ 문법 상 똑같이 사용 가능)
