...
int main()
{
    ...
    // - 이를 decay된다고 함. [배열] 그 자체는 길이 정보도 포함한 TYPE[N]형 !!!!!!!
    // - 단, sizeof(), typeid(), &주소연산자와 사용할 때, 레퍼런스를 바인딩 할 때 등등. decay에는 예외 상황이 있음 !!!!!!!
    // - 참고로 constant pointer 즉, 가리키고 있는 주소값이 고정된 포인터임 !!!!! (array++같은거 안됨)
    // - (이 때 TYPE은 배열 원소의 타입을 의미한다. 다차원 배열이라면 TYPE이 n-1차원 배열일 것이다)
    pointer = array;
