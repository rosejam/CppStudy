...
// 배열을 함수 인자로 넘기면, 컴파일러가 알아서 포인터로 치환하여 받는다 (TYPE[] -> TYPE*)!!!!!
...
int main()
{
    ...
    // 주소, test2[][][][][][][][][][][][]  <-값복사-  (.rodata영역의 ) [H][e][l][l][o][][W][o][r][l][d][\0]
    char test2[] = "Hello World"; // test2는 문자열의 길이 크기 (12바이트)
    // 다만 C++ 문법에서 [배열이름]은 <바구니 모음의 시작주소>을 나타낸다 !!!!!!! (위 포인터와 같다고 헷갈리게 하는 원인!!)
    // 즉, test2는 주소 !!!!! (디버거에서도 &test2뿐만 아니라 test2도!!! 주소값을 나타냄)
