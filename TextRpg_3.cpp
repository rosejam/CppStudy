...
// 포인터를 배웠으므로 플레이어와 몬스터를 전역변수 대신 스택 메모리의 지역변수 이용
// 나중엔 이를 Heap메모리에 올리게 될 것임
...
// PlayerInfo나 MonsterInfo는 위로는 못가고 아래로만 생명 주기가 유지된다!
...
void CreatePlayer(StatInfo* playerInfo); // info에 주소값만 넘김
void PrintStatInfo(const char* name, const StatInfo& info); // info에 참조값만 넘김 (마치 복사하듯이)
...
void CreatePlayer(StatInfo* playerInfo)
{
    ...
    while (!ready)
    {
        ...
        {
        case PT_Knight:
            playerInfo->hp = 100; // (*playerInfo).hp = 100;와 동일
...
void EnterGame(StatInfo* playerInfo)
{
    ...
    while (true)
    {
        ...
        PrintStatInfo("Player", *playerInfo); // *가 붙는다. 참조값을 넘겨야 하므로
